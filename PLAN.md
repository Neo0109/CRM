...
## Approved Radar relevance and domestic report preference (2026-09-15)

The user explicitly approved this independent Radar PR after diagnosis and preference selection. Use an independent snapshot of collected media and independent game-relevance scoring for all publishers; preserve demos, trailers, reviews and updates while excluding unrelated film/TV and general technology. Prefer direct gaming relevance, then 24h freshness, information and credibility, then publication time; max72h and no unknown dates. Remove the 16/16 regional fill targets while retaining caps 40 external, 3/source, 24/region, 12/topic, and 3 aggregate Bilibili. Group only evidence-confirmed events using original citations, bilingual entities and compatible action/object/time/version/facts; prefer complete valid domestic coverage, trying other domestic sources before foreign fallback if source caps are reached. Publisher origin comes from registry/domain, not story geography. Extend event checking to previous seven dates, excluding same-day; prioritize domestic metadata within the existing 90-second request budget. Existing sources/reference links only; no whole-web search or paid model. Preserve formal Leads and Steam behavior, public API/schema/UI and daily triggers. Validate real and fixed samples, recent replay, all tests and verify:all, then exact-head review, normal merge/deploy and real Radar/synced=true acceptance. Checkpoint: docs/checkpoints/radar-relevance-domestic-first.md. Stop after this PR's acceptance.


## Approved official gameplay metadata evidence (2026-09-15)

The user explicitly authorized this one independent sourcing PR: official Steam trailer categories and video-bound descriptions, verified same-game official Bilibili evidence, HLS/DASH plus legacy playback links, and targeted missing-gameplay refresh for cached candidates within the existing official lookup budget. No video analysis, frame extraction, AI calls, other gate relaxation, public API/UI change, workflow trigger change or direct CRM write. Freeze and inspect the 46 existing official-video candidates, run focused tests/Daily/verify:all, then normal PR review, squash merge and deployment acceptance. Keep evidence recovery, strict formal additions, near-pass reviews and actual CRM creation separate. Checkpoint: docs/checkpoints/official-gameplay-evidence.md.
