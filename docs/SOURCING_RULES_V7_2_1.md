...
One pure named-project extractor validates structured project fields first, then a quoted name, then a conservative explicit unquoted title. It ignores missing and non-string fields and rejects literal missing markers, generic category nouns, event-only phrases, and wholly generic descriptors after NFKC/case/whitespace/punctuation normalization. The generic boundary uses whole-string dynamic-program segmentation against one curated vocabulary covering qualifier/quantifier, organization/team, game/product/project, news/update/message, business/license/publishing, and English-equivalent tokens. A name is generic only when every normalized character can be covered by those tokens, closing compositional forms such as `全新游戏项目`, `一款全新游戏`, `某公司项目`, `公司最新消息`, and `latest game project`; normalized spelling such as `最新 消息` or `行业：资讯` cannot bypass it. Generic qualifiers `原创`, `自研`, `知名`, and `头部` are insufficient when they cover the whole name.

Count segmentation accepts an optional `第/约/近/超/超过/逾/至少/至多/最多/共/合计/累计/数` prefix, an Arabic or Chinese numeral, and a 款、个、部、项 classifier. It also recognizes whole generic batch labels `一批`, `一系列`, and `若干批`. Thus `第十款`, `数十款`, `约10款`, and `超过10款手游` cannot pass, while a distinctive name such as `纪元10：余烬` or `第七史诗` remains valid because it leaves non-generic residue.

Region, promotion, genre, and platform-only modifiers—including 国产、海外、首款、热门、二次元、策略、移动, and PC—are generic when they cover the entire proposed name. Bilibili aliases and organization-only labels are also insufficient. The organization predicate shares one runtime company vocabulary with the existing domestic-company sourcing signal, including 网易、腾讯、朝夕光年、莉莉丝、心动、鹰角、库洛、叠纸、沐瞳、灵犀、祖龙、完美世界, and 中手游. A company alone, the same company followed only by 旗下、网络、游戏、科技, a known media/source label such as 游戏日报, or a conservative 公司、集团、工作室、团队、企业、厂商、制作组 suffix shape is rejected. A real name such as `中国式家长`, `腾讯极光计划`, `莉莉丝深空计划`, `武侠乂`, `新月计划`, `行业动态模拟器`, `Project Echo`, or `New Game Chronicle` is retained because the full name has distinctive residue.
...
For an unquoted category-to-event slot, leading name introducers and trailing temporal/announcement connectors are stripped only at the slot edges. The category-first `新作` introducer may be glued to the name, so `国产手游新作星海远征公布 Demo` still binds exactly to `星海远征`; this trim is never applied globally. Chinese connectors include `预计/今日将/将/将会/计划于/有望/宣布将`; English framing includes `will/shall/plans to/is set to/expected to` plus bounded announce/reveal/launch forms. Both name-before-category and category-before-name forms must store the exact project.

Quoted extraction considers every `《…》` entity, rejects source/organization roles before distance ranking, and rejects wholly generic document/policy/report entities. Document-role suffixes include the existing 办法、条例、规范、白皮书、报告、备忘录、协议、通知、指南 families plus 意见、倡议、要点、决定、规划, and 纲要. Therefore `《星海远征》…《游戏日报》…Demo` binds `星海远征`, while a source-only `《游戏日报》` or `《关于促进网络游戏高质量发展的若干意见》` cannot become a project. The same extracted name is reused as the tagged media Lead project; identity-only admission may retain the legacy title fallback when no project name is required.
...
- generic region/promotion/genre/platform/prefix-count, Bilibili-alias, organization-only, source/document, and connector-only names remain excluded across structured, quoted, and unquoted paths;
