...
Count segmentation accepts the existing `第/约/近/超/超过/逾/至少/至多/最多/共/合计/累计/数` prefixes, an optional magnitude `上`, an Arabic or Chinese numeral, an optional `余/多/来` approximation suffix, and a 款、个、部、项 classifier. It also recognizes `成百上千` magnitude forms and whole generic batch labels `一批`, `一系列`, and `若干批`. Thus `第十款`, `数十款`, `上百款`, `10余款`, `至少上百款`, and `成百上千款手游` cannot pass, while a distinctive name such as `纪元10：余烬`, `余烬10`, or `第七史诗` remains valid because it leaves non-generic residue.

Region, promotion, genre, and platform-only modifiers—including 国产、海外、首款、热门、二次元、策略、移动, and PC—are generic when they cover the entire proposed name. Bilibili aliases and organization-only labels are also insufficient. The organization predicate shares one runtime company vocabulary with the existing domestic-company sourcing signal, including 网易、腾讯、朝夕光年、莉莉丝、心动、鹰角、库洛、叠纸、沐瞳、灵犀、祖龙、完美世界, and 中手游. A company alone or the same company followed only by role tokens such as 旗下、互娱、娱乐、互动、数字、文化、信息、软件、传媒、网络、游戏、科技 is rejected. A known media source or a bounded 新闻、日报、时报、周报、晚报、电视台、广播、通讯社、媒体、新闻网、资讯 suffix shape is also a source entity, not a project. Source/attribution prose ending in 消息、报道、显示、称、宣布、透露、指出, or 表示 is rejected across structured, quoted, and unquoted paths, including before-category forms such as `IT之家消息 国产手游…`. A real name such as `中国式家长`, `腾讯极光计划`, `莉莉丝深空计划`, `灵犀互娱：星火`, `新闻大亨`, `武侠乂`, `新月计划`, `行业动态模拟器`, `Project Echo`, or `New Game Chronicle` is retained because the full name has distinctive residue rather than only a role.
...
- generic region/promotion/genre/platform/count-magnitude, Bilibili-alias, organization-only, media/source/attribution/document, market/platform/industry, and connector-only names remain excluded across structured, quoted, and unquoted paths;
