...
Count segmentation consumes a complete generic quantity phrase from one shared grammar. Before the numeric core it accepts a repeatable sequence drawn only from the machine-declared quantity-operator tokens; these tokens compose totals, magnitude, approximation, comparison, and only forms rather than enumerating each complete prefix. The core is Arabic, Chinese including `几`, or indefinite `若干`, may carry one optional `至/到` second core, may use pre-classifier `余/多/来`, must end in a 款、个、部、项、批 classifier, and may use post-classifier `以上/以下/以内/左右/上下/起/余`. Separate bounded magnitude and series forms cover `成百上千`, `成千上万`, `一批`, `一系列`, and `若干批`. Thus compositional forms such as `共计10款`, `累计达10款`, `大约有十款`, `差不多十款`, `不超过十款`, `仅有十款`, and `10到20款` cannot pass or fall through to legacy taxonomy; category-prefix recognition reuses the exact same source. Vague tokens `少量/大量/海量/一众/大批/一些/少数/多数` are also generic when the remainder is only a game/product/project noun. Rejection still requires complete normalized coverage, so numeric titles such as `纪元10：余烬`, `第七史诗`, `十字军之王`, `百分百鲜橙汁`, `好久不见`, and `两点之间` retain distinctive residue.
