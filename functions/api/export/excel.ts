import { readLeads, readSettings, requireAccess, type Lead, type PagesContext } from "../../_lib/crm";
...
export const onRequestGet = async ({ request, env }: PagesContext) => {
  ...
  try {
    ...
    const configuredPassword = settings.excel_export_password ?? env.EXCEL_EXPORT_PASSWORD ?? null;
    if (!configuredPassword) return jsonResponse({ error: "请先在 CRM 设置里配置 Excel 导出密码" }, 409);
