...
export const onRequestPost = async ({ request, env }: PagesContext) => {
  ...
  try {
    ...
    if (!text && !attachments.length) {
      return json({ error: "请输入关键词、线索说明、链接或截图备注" }, 400);
    }
...
function buildSteamLead({ steamAppId, details, text, links, contacts, attachments, today }: {
  ...
  text: string;
  links: string[];
  contacts: NonNullable<Partial<Lead>["contact_methods"]>;
  attachments: AttachmentMeta[];
  today: string;
}): Partial<Lead> {
...
function buildManualLead({ text, links, contacts, attachments, today }: {
  ...
  return {
    ...
    amplification: "线索助手录入，待评估内容放大方式",
    priority_reason: inferPriorityReason(text, regionPriority),
    rule_fit: links.some(isGameLink) ? "有可验证游戏链接，待人工复核" : "缺少 Steam/SteamDB 链接，需补充可验证页面",
    verdict: "线索助手录入，待人工复核",
    next_action: links.length ? "打开链接复核画面和发行结构" : "补充 Steam/官网链接后再判断",
    first_seen: today,
    notes: assistantNotes(text, attachments)
  };
}
...
function extractContactMethods(text: string): NonNullable<Partial<Lead>["contact_methods"]> {
  const methods: NonNullable<Partial<Lead>["contact_methods"]> = [];
...
function inferCountry(text: string) {
  if (/中国|国内|国产|大陆|上海|北京|广州|深圳|成都|杭州|武汉|厦门|苏州|南京/i.test(text)) return "中国";
...
function inferRegionPriority(text: string, country: string): NonNullable<Partial<Lead>["region_priority"]> {
...
function inferPriority(text: string, regionPriority: NonNullable<Partial<Lead>["region_priority"]>): NonNullable<Partial<Lead>["priority"]> {
...
function inferBilibiliFit(text: string) {
  ...
  return "待评估：线索助手录入，需补充 B站适配判断";
}

function inferPriorityReason(text: string, regionPriority: NonNullable<Partial<Lead>["region_priority"]>) {
...
function dedupeContacts(values: NonNullable<Partial<Lead>["contact_methods"]>) {
  const deduped = new Map<string, NonNullable<Partial<Lead>["contact_methods"]>[number]>();
