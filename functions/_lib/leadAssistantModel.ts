...
import { fetchSteamAppDetails, type SteamAppData } from "./leadAssistantSteam";
import { hasImageDataUrl } from "./leadAssistantVision";

export type ContactMethod = Lead["contact_methods"][number];
export type AssistantLead = Partial<Lead>;
...
export type AiContactMethod = {
  type?: string | null;
  value?: string | null;
  note?: string | null;
};

export type AiExtractedLead = {
  project?: string | null;
...
export function normalizeAssistantInput(payload: LeadAssistantPayload) {
  const text = normalizeInputText(payload);
  const attachments = Array.isArray(payload.attachments)
    ? payload.attachments.filter((item) => item?.name || item?.type || item?.data_url)
    : [];
  const imageAttachments = attachments.filter(hasImageDataUrl).slice(0, 6);
  return { attachments, imageAttachments, text };
}
...
export function extractLeadAssistantSignals(text: string) {
  return {
    contacts: extractContactMethods(text),
    links: extractLinks(text),
    steamAppIds: extractSteamAppIds(text)
  };
}

export async function buildAiLead(
  raw: AiExtractedLead,
  context: { text: string; attachments: AttachmentMeta[]; ocrText?: string; searchSummary?: string; today: string },
  options: { loadSteamAppDetails?: (steamAppId: string) => Promise<SteamAppData | null> } = {}
  ...
  const loadSteamAppDetails = options.loadSteamAppDetails ?? fetchSteamAppDetails;
  const details = steamAppId ? await loadSteamAppDetails(steamAppId) : null;
...
export function buildSteamLead({ steamAppId, details, text, links, contacts, attachments, today }: {
  steamAppId: string;
...
export function buildManualLead({ text, links, contacts, attachments, today }: {
  text: string;
  links: string[];
  contacts: ContactMethod[];
  attachments: AttachmentMeta[];
...
export function normalizeAiContacts(values: AiContactMethod[], fallbackContact: string | null | undefined): ContactMethod[] {
  const methods: ContactMethod[] = [];
...
export function ensureContactMethods(contacts: ContactMethod[], steamAppId: string | null, links: string[]) {
