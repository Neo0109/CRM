...
import { collectRadarEdition, parseRadarFeedItems, parseChuappRadarItems, readRadarArticleMetadata } from "../automations/jobs/online_daily_v4_radar.mjs";
...
for (const source of rules.radar_sources) {
  try {
    ...
    assert.ok(meta.summary || example.summary, "source sample lacks a publisher summary");
    console.log(JSON.stringify({ source: source.name, status: "ok", articles: items.length, example: example.link, published_at: meta.published_at }));
  } catch (error) {
    failures++;
