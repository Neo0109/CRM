...
test("watchdog does not degrade health for low formal Lead volume", () => {
...
test("watchdog treats an empty V7 Lead pool as healthy after artifacts and sync succeed", () => {
...
test("watchdog requests recovery when V7 qualified and push counts diverge", () => {
  const rootDir = fixtureRoot(
    "2026-07-15",
    { push: 1, watch: 0, drop: 0 },
    { new_qualified_count: 2, push_pool_count: 2 }
  );
  const state = inspectDailyReport("2026-07-15", thresholds, { rootDir, ruleVersion: RULE_VERSION });
...
function fixtureRoot(date, counts, scanSummaryOverrides = {}) {
  ...
  writeJson(rootDir, `data/sourcing_candidates/${date}.json`, {
    ...
    scan_summary: {
      ...
      new_qualified_count: counts.push,
      push_pool_count: counts.push,
      ...scanSummaryOverrides
    },
