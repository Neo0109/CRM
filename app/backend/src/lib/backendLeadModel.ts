export type BackendBucket = "未处理" | "推进池" | "待评测" | "测试中" | "跟进中" | "观察池" | "淘汰池";
export type BackendStage = "new" | "watch" | "active" | "negotiating" | "won" | "rejected";
export type BackendPriority = "P0" | "P1" | "P2" | "P3";
export type BackendRegionPriority = "国内优先" | "海外-高视觉" | "海外-强数据" | "其他";
export type BackendRegion = "中国" | "海外";
export type BackendReviewStatus = "未处理" | "已查看" | "跟进中" | "已淘汰";
export type BackendContactType = "微信/QQ" | "Email" | "电话" | "官网" | "Steam" | "Discord" | "B站" | "X/Twitter" | "其他";
export type BackendEvaluationGrade = "S" | "A+" | "A" | "A-" | "B+" | "B" | "B-" | "C+" | "C" | "C-";

export type BackendContactMethod = {
  type: BackendContactType;
  value: string;
  note?: string | null;
};
...
export type BackendLead = {
  ...
  city: string | null;
  region_priority: BackendRegionPriority;
  ...
  contact: string | null;
  contact_methods: BackendContactMethod[];
  links: string[];
  ...
  risks: string | null;
  verdict: string;
  evaluation_grade: BackendEvaluationGrade | null;
...
export type BackendDailyReport = {
  report_date: string;
  summary: string;
  insights: string[];
  push_pool: Partial<BackendLead>[];
  watch_pool: Partial<BackendLead>[];
  drop_pool: Partial<BackendLead>[];
};

export type BackendNormalizeLeadOptions = {
  today?: string;
};

export type BackendImportStats = {
...
export type BackendMergeIncomingLeadsResult = {
  leads: BackendLead[];
  created: number;
  updated: number;
  dropped: number;
  total: number;
  import_stats: BackendImportStats;
};

const bucketValues: BackendBucket[] = ["未处理", "待评测", "测试中", "观察池", "跟进中", "推进池", "淘汰池"];
const reviewStatusValues: BackendReviewStatus[] = ["未处理", "已查看", "跟进中", "已淘汰"];
const contactTypes: BackendContactType[] = ["微信/QQ", "Email", "电话", "官网", "Steam", "Discord", "B站", "X/Twitter", "其他"];
const evaluationGrades: BackendEvaluationGrade[] = ["S", "A+", "A", "A-", "B+", "B", "B-", "C+", "C", "C-"];

export function normalizeBackendLead(raw: Partial<BackendLead>, options: BackendNormalizeLeadOptions = {}): BackendLead {
...
export function mergeBackendIncomingLeads(
  existing: BackendLead[],
  rawLeads: Partial<BackendLead>[],
  options: BackendNormalizeLeadOptions = {}
): BackendMergeIncomingLeadsResult {
  const byKey = new Map(existing.flatMap((lead) => backendLeadKeys(lead).map((key) => [key, lead.id] as const)));
  const byId = new Map(existing.map((lead) => [lead.id, lead]));
  let created = 0;
  let updated = 0;
  let dropped = 0;
  const import_stats: BackendImportStats = {
  ...
  for (const raw of rawLeads) {
    const incoming = normalizeBackendLead(raw, options);
    const matchId = backendLeadKeys(incoming).map((key) => byKey.get(key)).find(Boolean);
    if (matchId && byId.has(matchId)) {
      ...
      for (const key of backendLeadKeys(merged)) byKey.set(key, merged.id);
      updated += 1;
      trackUpdatedImport(import_stats, current, incoming);
    } else {
      byId.set(incoming.id, incoming);
      for (const key of backendLeadKeys(incoming)) byKey.set(key, incoming.id);
      created += 1;
  ...
  const leads = Array.from(byId.values()).sort((a, b) => {
    const bucketOrder: Record<BackendBucket, number> = { "未处理": 0, "待评测": 1, "测试中": 2, "观察池": 3, "跟进中": 4, "推进池": 5, "淘汰池": 6 };
...
export function mergeBackendLead(current: BackendLead, incoming: BackendLead): BackendLead {
  const keepCurrentWorkflow = shouldKeepCurrentWorkflow(current, incoming);
  return normalizeBackendLead({
    ...current,
    ...incoming,
    id: current.id,
...
export function backendLeadKeys(lead: BackendLead) {
...
export function backendLeadsFromReport(report: BackendDailyReport): Partial<BackendLead>[] {
  if (!report.report_date || !Array.isArray(report.push_pool) || !Array.isArray(report.watch_pool) || !Array.isArray(report.drop_pool)) {
    throw new Error("Invalid daily report");
  }
  return [
...
export function isBackendDailyReport(value: unknown): value is BackendDailyReport {
...
function trackCreatedImport(stats: BackendImportStats, incoming: BackendLead) {
...
function trackUpdatedImport(stats: BackendImportStats, current: BackendLead, incoming: BackendLead) {
...
function inferRegionPriority(country: string | undefined, signals: string | null | undefined): BackendRegionPriority {
...
function inferRegion(country: string | undefined): BackendRegion {
  return isDomestic(country) ? "中国" : "海外";
}

function inferPriorityReason(raw: Partial<BackendLead>) {
...
function inferRuleFit(raw: Partial<BackendLead>, country: string, links: string[]) {
...
function normalizeContacts(value: unknown, legacyContact: unknown): BackendContactMethod[] {
  const methods: BackendContactMethod[] = [];
  if (Array.isArray(value)) {
    for (const item of value) {
      if (!item || typeof item !== "object") continue;
      const record = item as Record<string, unknown>;
      const type = contactTypes.includes(record.type as BackendContactType) ? record.type as BackendContactType : "其他";
...
function inferContactType(value: string): BackendContactType {
...
function normalizeBucket(value: unknown): BackendBucket {
  return bucketValues.includes(value as BackendBucket) ? value as BackendBucket : "未处理";
}

function normalizeReviewStatus(value: unknown, bucket: BackendBucket): BackendReviewStatus {
  if (bucket === "未处理") return "未处理";
  if (reviewStatusValues.includes(value as BackendReviewStatus)) return value as BackendReviewStatus;
...
function normalizeEvaluationGrade(value: unknown): BackendEvaluationGrade | null {
  return evaluationGrades.includes(value as BackendEvaluationGrade) ? value as BackendEvaluationGrade : null;
}

function stageFromBucket(bucket: BackendBucket | undefined): BackendStage {
...
function priorityFromBucket(bucket: BackendBucket | undefined): BackendPriority {
  if (bucket === "推进池" || bucket === "跟进中" || bucket === "测试中") return "P1";
  if (bucket === "淘汰池") return "P3";
  return "P2";
}

function priorityOrder(priority: BackendPriority) {
  return { P0: 0, P1: 1, P2: 2, P3: 3 }[priority];
}

function reviewOrder(status: BackendReviewStatus) {
...
function mergeContactMethods(current: BackendContactMethod[], incoming: BackendContactMethod[]) {
  const deduped = new Map<string, BackendContactMethod>();
...
function csvCell(value: unknown) {
  if (Array.isArray(value)) {
    if (value.every((item) => item && typeof item === "object" && "value" in item)) {
      return csvCell((value as BackendContactMethod[]).map((item) => `${item.type}:${item.value}${item.note ? ` (${item.note})` : ""}`).join(" | "));
    }
